...
该命令加载独立的 `16 m × 12 m` 竞赛演示场；橙色外框是 30 m² 外部任务区，青色内框是 12 m² 实际清扫区，覆盖率只统计内框。手动演示默认不加载 CameraTracking，可在 3D 区域自由旋转、平移和缩放。小场以约 31% 相邻条带重叠、刷盘前置偏移补偿和真值漏点自动补扫消除跟踪误差造成的漏扫，并要求实际覆盖率至少 99.5%。右侧实时显示清扫指标、规划/实际轨迹和已清扫面积。默认目标 2x，可传 `-SimulationSpeed normal|turbo`，实际 RTF 以顶部 World Stats 为准。
