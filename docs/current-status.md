...
10,440 m² 长直道候选现使用 skip-lane 顺序、连续简单 Dubins 跟踪、目标清扫带刷盘关闭引导重叠和 `0.20 m/s` 绝对曲率限速；限时 Gazebo 诊断已连续跨过多个 U 形连接器。双天线 GNSS 航向融合也使若干单次诊断的 XY RMSE/P95 低于 `0.05 m`，但同步配对样本不足，且没有整场与 30-seed 报告，不能计作正式 B/C/D 证据。未知栅格 frontier 链已在 40 m × 20 m 烟测中完成 995.34 m² 建图、地图与位姿图保存、硬重启、加载重定位和 5 航点导航，两阶段 TF 断裂均为 0；该证据明确标记 `formal_scope=false`。全范围开发链现使用独立长前视控制、物理可达转弯、60 s 目标看门狗、180 s 失败记忆、边界耗尽恢复、Nav2 原生碰撞检查倒车，以及仅排序在线 frontier 的边界/激光量程推导蛇形偏置。横向扫带要求在线地图观测正式 x 边界；垂直换带先用碰撞检查 BackUp 退离边界，再从当前融合位姿、在线 costmap 和扫带方向生成短距已知自由区 staging 候选。连接器由 Smac Hybrid 只规划一次，经在线 costmap 全路径净空复核后按前进/倒车 cusp 分段，分别交给 forward-only `DubinsPath` 和 `ReversePath`，避免 BT 重规划或允许倒车的通用控制器在静止车位改变起始档位。当前最佳正式口径限时证据为 1200.39 s 建图 4958.44 m²（4.13 m²/s，114 成功/4 失败），地图横向真实跨过 x=±100 m、TF 断裂为 0，较旧 3741.1 m² 提升约 32.5%。但 plan-once 换带的 6/4/2 m 在线候选均在 forward 分段触发 Nav2 进度失败，纵向地图边界仍停在约 32 m；所以 20,000 m² 全程及后续保存、硬重启、加载重定位、多航点正式链未执行，仍不能计作正式 B 证据。完整感知、Tracking、Spot Cleaning 与 re-observation 继续受下述分类器停止条件阻塞。
