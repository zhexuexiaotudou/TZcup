...
- 可行性结论为“高保真名义数字样机可行、A300条件可行、全场粒子流体和未经实物标定的单位级数字孪生不可承诺”。用户已确认允许依据公开数据表自建并开源缺失CAD/Xacro；当前只完成方案文档，没有下载模型、制作URDF/CAD、编写插件或运行新仿真。
- 随后完成[URDF实施前准备包](pre-urdf-readiness.md)：5个开源仓库锁定commit，冻结A300/UR5e/2F-85、8个传感器、两套独立Arducam鱼眼、带编码器Pololu刷盘电机、Actuonix推杆和Jabsco回收泵，并建立24 frame、8 joint、功耗和许可证契约。预载荷为`77.705 kg`，20个最坏铝块为`1.512 kg`，仅按载荷反算的污水名义/可用/正常episode上限为`12.133/9.706/8.493 L`；重心、安装空间、S100实板包络和VBAT拓扑仍是正式URDF阶段硬门。本轮没有制作正式Xacro/mesh或运行新仿真。
