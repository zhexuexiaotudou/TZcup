...
- MoveIt 正式 launch 已接入独立 planning-scene bootstrap：正式 SRDF 新增只由定位 TF 驱动的 planar `map -> base_footprint` virtual joint，机械臂 group 仍不包含底盘；配置化 `ground` 在 MoveIt planning frame `map` 中以 `(98,0,-0.25)` 为中心、尺寸 `220 x 120 x 0.50 m`，覆盖一次定位变换后的正式 geofence `[-2,198] x [-50,50]` 并四边各留 `10 m`，不再使用会随车辆移动失效的局部 patch。仅四个 wheel-support link 通过 ACM 允许正常接地；ground、ACM 和单调 revision 均须由 `GetPlanningScene` 回读，执行器在 scene ready 前不得发命令。bootstrap 的 timer、Apply/Get clients 和 TF listener 使用可并发且不重入的 callback-group 结构，避免等待异步服务时死锁；live gate 还要求 `multi_dof_joint_state` 中的 virtual joint 与 fresh `map -> base_footprint` TF 一致。`halton_00772` 负态必须由 `GetStateValidity` contacts 明确归因 `ground` 与非豁免 arm link；持久性负测只用 `is_diff=true + REMOVE ground`，并要求 bootstrap 恢复后非 ground world/ACM 不变，绝不发送会清空 robot state/ACM 的空 full-scene reset。正常 anchors、ground 恢复和 perceived-cube 更新后的实时验证仍待 Linux/MoveIt，纯函数测试不冒充运行通过。
- 场景生成器现按真实旋转 box/cylinder 的二维 collision footprint 校验整段行人胶囊路径，并把实体 cube 纳入硬碰撞；每条新路径还对全部既有行人的完整 segments 做 exact 2D capsule-distance 检查，中心线距离 `<=0.50 m`（两个 `0.25 m` 半径之和）即 fail closed。dirt、puddle、leaf 保持地表语义，既有 `static=true` 与 10 Hz `SetEntityPose` 技术路线不变。修复前公开审计在 `22,400` 对路径中发现 `251` 对违规、影响 `216/800=27.00%` episode；修复后使用 Python runner 重新生成并审计全部公开 `32 train + 8 val` 地图、每图 mission `0..19`，共 `800 episodes / 6400` 条行人路径 / `22,400` 对路径：行人-静态 collision `0`、行人-cube collision `0`、行人-行人路径违规 `0/22,400`，对象计数、`20000 m2` 面积和 seed determinism 均通过，`hidden_accessed=false`。
- r064 首次建图 live 运行暴露了另一个确定性配置错误：mapping 明确关闭高带宽传感器，但 collision monitor 仍等待没有 publisher 的 `mid360`，表现为 `/cmd_vel_nav` 有命令而 `/cmd_vel_gate` 无样本、safety `command_timeout`、覆盖仅 `1.24145%`。mapping materialization 现只保留实际在线的 `/scan/navigation`，cleaning/high-bandwidth 模式不变；runtime collector/validator 还要求真实 Nav→smoother→gate→whole-vehicle-safety 非零链，逐段 sole publisher 必须依次为 `/controller_server`、`/velocity_smoother`、`/collision_monitor`、`/whole_vehicle_safety_manager`，并要求 active-command safety 无 `command_timeout` 和既有 `>=0.10 m` odom 位移。旧 r064 地图/运行证据不得复用，修复效果仍须在 fresh R065 Linux build/run-root 中重跑。
- public manifest 明确区分 source-world geofence、一次固定起点变换后的 localization-map geofence 和机器可读 legacy 迁移语义；materializer、map lifecycle、research harness 与正式脚本 consumer 对未知 frame、未平移或重复平移均 fail-closed。地图分辨率按实际用途分层：静态 materializer 来自 launch/实参（默认 `0.10 m`），lifecycle support mask 来自函数实参（默认 `0.25 m`），SLAM resolution 只从 saved-map runtime metadata 读取并要求 `<=0.10 m`，coverage raster 默认 `0.25 m`；不再声称所有地图共享一个分辨率。
- 能力边界同步为：校园 `puddle` 只是 perception/coverage 的无碰撞视觉表面，不证明水深/水量/回收；静态 mesh-ray、离散 arm anchors 和 deterministic URDF 检查不外推为连续全空间、真实标定、真实载荷或 S100P 集成。首次建图和普通安全配置仍为 `0.45 m/s`；`1.0 m/s` 仅是显式 dry requalification candidate，须依次通过四个运行门和实测效率门，本变更不授权实机提速。S100P 尺寸/质量/热/标定、A300 实物减重及连续机械臂空间继续保持 external blocked。
...
- 修正干湿箱、投放通道和后车身碰撞：干箱可用容积 40.116775 L、污水箱有效容量 9.7064 L，车身侵入均为 0；30 mm 方块投放路径最小净空 73.64 mm。机械臂验收使用同一个 PET 刚体完成双指接触、0.260000 m 抬升、释放、落箱和持续承载接触，不在任务中删除或重置垃圾实体。
- 收紧工程真值链：刮条和吸口的布局/部件台账坐标已同步到实际接地 Xacro；正式 `use_sim:=true` URDF、布局/产品/部件报告改由原子生成器统一产出，并用 source/output SHA-256 manifest 在 Windows fast CI 中阻止“源模型已变、提交快照未变”的假绿。正式快照不再携带任意开发机的绝对安装路径。
- `opennav_coverage@2241180` 的上游路径测试原先按当前秒随机生成场地，曾偶发触发 native `free(): invalid pointer`；项目保留精确上游基线，只对该测试应用一行固定 seed 补丁，并以确定性本地 commit、patch/tree/diff SHA-256、唯一文件变更和 clean checkout 双阶段审计，禁止用重试或跳过掩盖失败。
- 新增四场景联合验收：前进/停车、正常积水回收、满箱闭锁、机械臂抓取投箱。每场景使用独立 ROS domain 与 Gazebo partition，并要求进程残留为 0；总清单仅在四项均通过时原子发布。全新三包 colcon 工作区逐包记录构建标记，launch/Xacro/world/Python 安装文件逐项与源码 SHA-256 对照，两个 Gazebo 插件各要求恰好一份。
- `0360f63` 的统一实跑全部通过：Gazebo 真值前进 1.012341 m、控制器里程 0.994900 m、停车后真值滑移 0.007055 m；正常场回收 2.88 L、24/24 列、回收率 100%、质量误差 0；满箱停在 9.7064 kg，保留 0.3436 L 地面积水且后续扣减为 0；PET 方块质量 0.03726 kg，抬升 0.260000 m，腕部/方块相对位姿变化 1.81 µm，落箱后 4.0 s 稳定观察并得到 12,396 次箱体承载接触。总清单 SHA-256 为 `9D28964B800A67F074EF53C6F9E00CAE3DF5EB46890E5A4409CD31FCC14DC210`。
- 当前结论只关闭正式整车的基础机构物理门，不等于 MoveIt 生产抓取、随机多垃圾/多水斑任务、200 m × 100 m 园区建图、DOSOD+EdgeSAM、RL/Nav2 综合闭环、跨场地泛化或 S100/征程6 实板部署通过。
...
- 机械臂承力链补齐载荷平台背板、加强 pedestal 和六螺栓转接盘。六轴 UR5e 与 2F-85 主驱动拆分为独立 `arm_controller` / `gripper_controller`，禁止部分目标并设置轨迹/终点误差；Bullet Featherstone 中实际执行四段六轴轨迹和夹爪 `开→闭→开→半闭`，Action 均返回成功，六轴运动范围 `0.2175～0.3989 rad`、夹爪 `0.6443 rad`。提交内基线报告的最大终点误差为 `2.34e-5/4.25e-5 rad`；合并版 `main` 独立部署复验为 `1.42e-3/9.83e-4 rad`，仍低于控制门限。
