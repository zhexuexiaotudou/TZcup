...
DDRV4-06 首先在现有 OPRV3 moving-camera harness 上完成 24 mission、2160 帧全量兼容回归。运行时真实使用 RTX 4080、MMDetection 3.3.0、CUDA PyTorch 与 CUDA ORT；v1–v5 的 provider、cuDNN、Area 键和 NMS 集成失败均保留，v6 才完整结束。D1-B 的离散 eventual recall 仅 `0.3898`，metal `0.1053`、paper `0.70`、small `0.3529`，wrong-actionable `0.3510`；产品地图 precision/coverage 仅 `0.2111/0.19`。该回归明确失败并阻止 freeze，但它缺 behind-FOV、turn、occlusion、reflection，且不是完整 G7 moving pack，因此 `DDRV4_06_FORMAL_COMPLETE=false`：既不能宣称在线通过，也不能把旧任务单独冒充 DDRV4 最终在线裁决。下一步必须补齐 G7 moving missions；在此之前 `DDRV4_X86_DEV_PASS=false`、G5_V2 未打开。紧凑证据见 [`DDRV4_ONLINE_DEV_SUMMARY.json`](../artifacts/detector_data_recovery_v4_20260811T134117Z/online_dev/DDRV4_ONLINE_DEV_SUMMARY.json)。

DDRV4 产品性能回放补齐了哈希绑定的 D1-B MMDetection 路径，在 300 帧、10 Hz 输入上真实执行检测器 300 次、leaf/puddle 各 150 次、tracker、动态地图与调度队列。结果为 `9.9974 Hz`、P95 `155.83 ms`、掉帧率 `0`；严格 `>=10 Hz` 门因低 `0.0026 Hz` 而失败。该性能结果不改变在线质量回归的内部阻断。J6/field 资源复扫确认无冻结 student、当前安装工具链、实体板、RGB-D 录制或独立 map GT；现有现场采集和审查软件就绪。DDRV4-14 因此前置门失败只生成 fail-closed 最终状态、blocker、模型注册、release 空清单、许可说明和 evidence index，不运行 neat-freak、Ready、merge 或部署。
