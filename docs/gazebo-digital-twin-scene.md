...
可清扫目标共 10 个，瓶、罐、纸张、纸盒和落叶各 2 个，均匀分布在实际清扫区的不同条带上。为保证录制时可以从同一斜俯视角直接数清，目标采用与默认视线正交、远离车辆起点且彼此不重叠的两行五列布局；纸张增加同尺寸超薄实体底层和紧贴轮廓的深色薄边，每堆落叶使用六片真实尺度、可双面观察的薄实体叶片和不规则贴地阴影。这里没有圆形高亮、悬浮光环或额外碰撞体。
