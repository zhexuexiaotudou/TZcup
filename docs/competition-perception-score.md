...
训练后又在同一 RTX 3080 Ti 上做了 5000 次固定输入推理，用时 `0.9065 s`（`5515.63 inference/s`）；GPU PyTorch 与 ONNX Runtime CPU 的最大 logit 差为 `1.24e-05`，argmax agreement 为 `1.0`。该检查只证明 GPU 推理可用和导出数值一致，不代替冻结 holdout 或官方验收。
