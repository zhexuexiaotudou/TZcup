...
- 正式 W1 runner 必须先持续发布独立 base inhibit，并确认 fresh `INHIBITED + manipulator_base_inhibit`；随后只以 `10 Hz` 发布物理仿真 operator 的急停释放、急停复位和主电源三路 Bool 心跳，且逐 topic 要求唯一 publisher。只有 safety 序号继续前进并达到精确 `BASE_COMMAND_STOPPED + manipulator_base_inhibit` 后才能启动 footprint gate。退出时必须先停止急停释放/复位 publisher，再持续发布物理急停并读回 fresh `INHIBITED + manual_estop`，最后停止主电源、急停和 inhibit publisher；该序列不得发布 `cmd_vel`、关节/执行器命令或 evaluator truth，也不得把 `INHIBITED` 放宽为 W1 正向验收状态。
