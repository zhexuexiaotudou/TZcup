...
| 清扫机构 | 双侧刷、中央滚刷、P16 升降、带约 11.9 N 名义预载的双自由度弹簧阻尼浮动刮条、吸口、三段软管、过滤器 |
...
- L1 有限 2.5D 水层的历史门已通过刮条/吸口几何、守恒回收和满箱闭锁；当前快照仍须正式会话重跑。被动刮条已加入有界弹簧阻尼、约 11.9 N 名义预载、刀片接触传感器，以及模型施加的弹簧阻尼 effort/运动遥测；后者不是实测关节反力或直接接地证据，直接刀片-地面接触只由正常积水回收的 ContactSystem 门判定。刷毛连续接触力、软管柔性、污水自由液面和飞溅仍需后续增强；
