...
静态模型门的证据等级保持分层：确定性 URDF 质量/惯量检查只证明展开模型的数值合同和单树结构；mesh-ray FOV 只证明冻结姿态、冻结网格与采样射线下的静态无遮挡比例；运输、预抓、抓取和投箱 anchor 扫描只证明列举的离散姿态。三者都不能外推为连续机械臂全关节空间、运动中自遮挡、真实镜头标定、真实整车载荷/重心或 S100P 机械与热集成通过。
