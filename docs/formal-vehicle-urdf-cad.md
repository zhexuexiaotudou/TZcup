...
`base_footprint` 表示。校验器会在零关节位姿下正向求解整棵树，并要求 29 个非 pending
...
`4.25e-5 rad`。正式启动器使用 Gazebo 官方 mimic 示例要求的 Bullet Featherstone
引擎；最新 `/joint_states` 只暴露夹爪主驱动关节，因此不把五个 mimic 从关节的运动误写成
独立测量结果。合并版部署验收以独立进程组启动，SIGINT/TERM 后确认无本轮 Gazebo 残留。
