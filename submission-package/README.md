...
当前整体未达到完整提交条件。技术报告、板端证据与结果接入工具已成稿；已有真实Gazebo GUI partial clip，但没有有效5-10分钟完整任务Demo或可用MCAP，且没有伪造视频或未测PASS。
...
- `evidence/day1/demo-failure-retention.json`：两次预Gazebo失败和两次有界 full-task failed attempt的提交、回滚点与哈希台账；不声称包含可用MCAP。
