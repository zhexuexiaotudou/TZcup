...
if args.intake:
    ...
    for update in incoming:
        if update['id'] not in known or update['status'] not in ['PASS','FAIL','NOT_MEASURED']:
    ...
    for update,files in planned:
        ...
        for k in ['status','value','basis','note','run_id','source_commit']:
...
for r in rows:
    assert r['status'] in ['PASS','FAIL','NOT_MEASURED']
...
for page,(title,paras) in enumerate(sections,1):
    ...
    c.drawRightString(550,43,f'{page} / 20'); c.showPage()
c.save()
(P/'docs/技术方案报告.md').write_text('\n\n'.join('# '+t+'\n\n'+'\n\n'.join(ps) for t,ps in sections),encoding='utf-8')
print(json.dumps({'pages':20,'counts':{s:sum(r['status']==s for r in rows) for s in ['PASS','FAIL','NOT_MEASURED']}},ensure_ascii=False))
