...
触发前 GT 线速度 **1.000026 m/s**，角速度 **0 rad/s**。触发时刻 **54.051 s**；GT 源时间戳 **54.540 s** 起，三维线速度模与三维角速度模均不超过 **0.01 m/s、0.01 rad/s**。事前约定的停止保持判据为 **1.0 s**；本次数据实际观测连续保持 **1.5 s**，比判据多出 0.5 s。仿真延迟仍为 **0.489 s**，PASS 不变。
