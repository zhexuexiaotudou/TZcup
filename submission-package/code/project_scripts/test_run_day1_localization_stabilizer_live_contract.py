...
def test_live_harness_is_serialized_and_fail_closed():
    source = (ROOT / "run_day1_localization_stabilizer_live.sh").read_text()
...
def test_runtime_sources_do_not_use_ground_truth_for_control():
    files = [
        ROOT.parent
        / "ros2_ws/src/sanitation_localization/sanitation_localization/map_odom_stabilizer.py",
        ROOT.parent
        / "ros2_ws/src/sanitation_localization/sanitation_localization/map_odom_stabilizer_core.py",
    ]
    for path in files:
