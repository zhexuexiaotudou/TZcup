...
const ComplianceAxisParameters kFloat{1800.0, 45.0, -0.006, 120.0};

TEST(SqueegeeComplianceCore, AppliesSpecifiedGroundPreloadAtZeroDatum)
{
  EXPECT_NEAR(SqueegeeComplianceCore::Effort(kFloat, 0.0, 0.0), -10.8, 1e-12);
}

TEST(SqueegeeComplianceCore, RelaxesAtFreeSpringReference)
{
  EXPECT_NEAR(SqueegeeComplianceCore::Effort(kFloat, -0.006, 0.0), 0.0, 1e-12);
}

TEST(SqueegeeComplianceCore, DampingOpposesJointVelocity)
{
  EXPECT_NEAR(SqueegeeComplianceCore::Effort(kFloat, -0.006, 0.1), -4.5, 1e-12);
  EXPECT_NEAR(SqueegeeComplianceCore::Effort(kFloat, -0.006, -0.1), 4.5, 1e-12);
}
