...
TOPICS = ("/camera/color/image_raw", "/camera/depth/image_rect_raw", "/ground_truth/semantic/image", "/ground_truth/instance/image")
...
def main() -> None:
    ...
    class Collector(Node):
        def __init__(self):
            super().__init__("stage5br3_g2_collector")
            ...
            self.started = time.monotonic(); self.publisher = self.create_publisher(Twist, "/cmd_vel", 10)
            for topic in TOPICS:
                self.create_subscription(Image, topic, lambda msg, key=topic: self.receive(key, msg), qos_profile_sensor_data)
            self.create_subscription(CameraInfo, "/camera/color/camera_info", lambda msg: setattr(self, "camera", msg), qos_profile_sensor_data)
            self.create_subscription(Odometry, "/ground_truth/model_odom_raw", lambda msg: setattr(self, "odom", msg), 20)
        ...
        def save(self, stamp, messages, pose):
            ...
            paths["tf"].write_text(json.dumps({"world_to_base_xy": list(pose[:2]), "base_to_camera_xyz_m": [0.53, 0.0, 0.22], "optical_frame": "camera_depth_link"}, indent=2)+"\n")
    ...
    report = {"schema_version": 1, "scene_seed": scene["scene_seed"], "world_id": scene["world_id"], "split": scene["split"], "requested_frames": args.frame_count, "captured_frames": len(node.saved), "records": node.saved, "adjacent_motion_gate_pass": len(node.saved) == args.frame_count, "capture_pass": len(node.saved) == args.frame_count and all(item["exact_four_sensor_timestamp"] for item in node.saved)}
