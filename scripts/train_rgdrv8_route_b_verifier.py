...
def scores(model,rows,root,device,batch=64):
    import torch; result=[]; model.eval()
    with torch.inference_mode():
        for off in range(0,len(rows),batch):
            part=rows[off:off+batch]; x=torch.from_numpy(np.stack([load_crop(root,r) for r in part]).transpose(0,3,1,2).astype(np.float32)/255).to(device); prob=torch.softmax(model(x),1).cpu().numpy()
...
def main():
    ...
    rng=random.Random(20260813); count=max(len(x) for x in pools.values()); balanced=[]
    for rows in pools.values(): rng.shuffle(rows); balanced += [rows[i%len(rows)] for i in range(count)]
    rng.shuffle(balanced); device=torch.device('cuda:0'); model=CandidateCropClassifier().to(device); opt=torch.optim.AdamW(model.parameters(),lr=3e-4,weight_decay=1e-4); loss_fn=torch.nn.CrossEntropyLoss(); a.output.mkdir(parents=True); history=[]; best=None
    for epoch in range(1,a.epochs+1):
        model.train(); order=list(range(len(balanced))); rng.shuffle(order); total=0
        for off in range(0,len(order),a.batch_size):
            rows=[balanced[i] for i in order[off:off+a.batch_size]]; x=torch.from_numpy(np.stack([load_crop(a.crops_root,r) for r in rows]).transpose(0,3,1,2).astype(np.float32)/255).to(device); y=torch.tensor([r['class_id'] for r in rows],device=device); opt.zero_grad(set_to_none=True); loss=loss_fn(model(x),y); loss.backward(); opt.step(); total+=float(loss)*len(rows)
        hold_scores=scores(model,hold,a.crops_root,device); sweep=[evaluate(hold_scores,x/100) for x in range(30,100,2)]; selected=max(sweep,key=lambda r:(r['gate_pass'],r['macro_f1'],r['background_specificity'],r['threshold'])); row={'epoch':epoch,'loss':total/len(balanced),'selected':selected}; history.append(row)
    ...
    checkpoint=torch.load(a.output/'verifier.pt',map_location=device,weights_only=True); model.load_state_dict(checkpoint['state_dict']); final_scores=scores(model,hold,a.crops_root,device); (a.output/'holdout_scores.json').write_text(json.dumps(final_scores,indent=2)+'\n'); selected=evaluate(final_scores,float(checkpoint['threshold']))
    report={'schema_version':1,'stage':'RGDRV8-03-ROUTE-B-VERIFIER','architecture':'torchvision_mobilenet_v3_small_imagenet1k_v1','classes':CLASSIFIER_CLASSES,'unique_train_counts':dict(Counter(r['class_name'] for r in train)),'balanced_exposures_per_class':count,'epochs':a.epochs,'selected_epoch':checkpoint['epoch'],'selected_threshold':checkpoint['threshold'],'holdout_metrics':selected,'checkpoint_sha256':sha(a.output/'verifier.pt'),'VAL_NEW_read':False,'G5_V2_read':False,'ROUTE_B_VERIFIER_HOLDOUT_PASS':selected['gate_pass']}
