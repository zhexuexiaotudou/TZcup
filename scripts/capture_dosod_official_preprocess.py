...
def capture(*,pilot_manifest:Path,pilot_record_index:int,images_y:Path,images_uv:Path,adapter_binary:Path,adapter_source:Path,dpkg_output:Path,stdout:Path,stderr:Path,command:list[str],returncode:int,output:Path,test_fixture:bool=False,dpkg_returncode:int=0,execution:dict|None=None)->dict:
 fresh_directory(output,'official_capture_output')
 r={"receipt_id":RECEIPT_ID,"status":"BLOCKED","test_fixture":test_fixture,"raw_sensor":None,"official_preprocessor":None,"execution":execution,"inputs":[],"producer_script_path":str(Path(__file__).resolve()),"producer_script_sha256":sha256_file(Path(__file__).resolve()),"started_epoch_ns":time.time_ns(),"ended_epoch_ns":None,"blockers":[]}
 try:
