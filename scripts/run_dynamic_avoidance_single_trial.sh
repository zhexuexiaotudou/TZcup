...
mission_crossing_count="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["schedule"]["mission_corridor_crossing_count"])' "${protocol}")"
corridor_fraction_start="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["schedule"]["corridor_fraction_range"][0])' "${protocol}")"
corridor_fraction_end="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["schedule"]["corridor_fraction_range"][1])' "${protocol}")"
...
export FORMAL_DYNAMIC_MISSION_CORRIDOR_CROSSINGS="${mission_crossing_count}"
export FORMAL_DYNAMIC_MIN_MISSION_CORRIDOR_CROSSINGS="${mission_crossing_count}"
export FORMAL_DYNAMIC_CORRIDOR_FRACTION_START="${corridor_fraction_start}"
export FORMAL_DYNAMIC_CORRIDOR_FRACTION_END="${corridor_fraction_end}"
