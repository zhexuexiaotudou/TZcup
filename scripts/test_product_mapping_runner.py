...
def test_formal_runner_has_real_restart_and_20k_fail_closed_scope():
    ...
    assert "-p no_progress_endpoint_success_limit:=3" in text
    assert "-p no_progress_raw_frontier_success_limit:=12" in text
    assert "-p no_progress_endpoint_exclusion_ttl_sec:=60.0" in text
...
def test_frontier_timeout_restarts_nav2_before_next_goal():
    ...
    assert '"frontier_no_progress_endpoint_exclusion_count"' in explorer
    assert '"frontier_no_progress_raw_exclusion_count"' in explorer
    assert '"frontier_success_without_map_progress_endpoint_excluded"' in explorer
