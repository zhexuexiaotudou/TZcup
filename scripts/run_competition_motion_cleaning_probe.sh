...
PY
launch_pid=''; nav_pid=''; filter_pid=''; bridge_pid=''; recorder_pid=''
cleanup() {
  if [[ -n "$recorder_pid" ]] && kill -0 "$recorder_pid" 2>/dev/null; then
    kill -TERM -- "-$recorder_pid" 2>/dev/null || true
    for _ in {1..100}; do kill -0 "$recorder_pid" 2>/dev/null || break; sleep 0.1; done
  fi
  formal_runtime_cleanup_groups "$GZ_PARTITION" "$nav_pid" "$filter_pid" "$bridge_pid" "$launch_pid" || true
}
...
setsid ros2 launch sanitation_formal_campus_integration formal_campus.launch.py \
  ...
  high_bandwidth_sensor_runtime:=false enable_training_gt:=false \
...
set +e
timeout --signal=TERM --kill-after=10s "$(( ${PROBE_SECONDS:-120} + 450 ))s" \
  python3 "$DRIVER" --output "$OUTPUT" --seconds "${PROBE_SECONDS:-120}" --prepare-seconds 420 \
