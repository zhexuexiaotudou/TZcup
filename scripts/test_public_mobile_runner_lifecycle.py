...
def _run_cleanup_fixture(*, stop_rc: int) -> tuple[int, list[str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="mobile-runner-test-", dir=work) as raw:
        ...
        "formal_runtime_stop_memory_watchdog(){ return 0; }; formal_runtime_memory_watchdog_tripped(){ return 1; }; FORMAL_RUNTIME_MEMORY_WATCHDOG_RESULT=0; FORMAL_RUNTIME_MEMORY_BREACH_EXIT_CODE=86\n"
        ...
        "RUN_ROOT=\"$(cd -- \"$(dirname -- \"${BASH_SOURCE[0]}\")\" && pwd -P)\"; ROOT=\"$RUN_ROOT\"; git(){ [[ \"$*\" == *status* ]] || printf fixture; }; SELECTOR=\"$RUN_ROOT/no-selector\"; DESIRED_STATE=BLOCKED; RUNNER_EXIT_CODE=4\n"
        "launch_pid=777; operator_pid=999999; collector_pid=''; stop_estop_pid=''; QUOTA_PID=''; DEADLINE_PID=''; PREFLIGHT_PID=''; PREFLIGHT_PGID=''; READINESS_PID=''; FINAL_ORACLE_PID=''; ADMISSION_BINDING_SHA256=fixture; ADMISSION_GIT_HEAD=fixture; ADMISSION_GIT_TREE=fixture; GIT_STATUS_AT_ADMISSION=''\n"
