...
def test_token_is_root_bound_and_rejects_another_cap(tmp_path: Path) -> None:
    run_root = tmp_path / "fresh"
    run_root.mkdir()
    token_path = run_root / "token.json"
    token = MODULE.create(profile_path=PROFILE, run_root=run_root, output=token_path)
    assert token["nonce"]
    assert MODULE.validate(
        profile_path=PROFILE, run_root=run_root, token_path=token_path, requested_cap=1.0
    )["nonce"] == token["nonce"]
    ...
    with pytest.raises(ValueError, match="does not bind this requalification run root"):
        MODULE.validate(
            profile_path=PROFILE, run_root=other_root, token_path=token_path, requested_cap=1.0
        )
