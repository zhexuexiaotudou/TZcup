...
def _validate(
    *, contract_path: Path, preflight_path: Path, config_path: Path,
    identity_path: Path, compiler: str,
...
def execute_compile(
    *, contract_path: Path, preflight_path: Path, config_path: Path,
    identity_path: Path, output: Path, compiler: str = "hb_compile",
    ...
    receipt: dict[str, Any] = {
        ...
        "inputs": {"contract": str(contract_path.resolve()), "preflight": str(preflight_path.resolve()), "compile_config": str(config_path.resolve()), "compiler_identity": str(identity_path.resolve())},
    ...
    try:
        contract, preflight, identity, expected_hbm, blockers = _validate(
            ...
            identity_path=identity_path, compiler=compiler,
        )
        receipt["inputs"].update({
            ...
            "calibration_manifest_sha256": preflight.get("calibration", {}).get("manifest_sha256"),
        })
...
def main() -> int:
    ...
    try:
        receipt = execute_compile(contract_path=args.contract, preflight_path=args.preflight_report,
                                  ...
                                  output=args.output, compiler=args.compiler)
    except Exception as exc:
