...
DOMAIN = "g10_physical_close_range_route_v15"
SPLIT_MAP = {
    "train": "G10_TRAIN",
    "val": "G10_HOLDOUT",
...
def main() -> int:
    ...
    for row, split, is_negative in all_rows:
        ...
        new_id = f"g10v15_{split}_{old_id.removeprefix('g4_')}"
    ...
    for split, group in world_groups:
        for local_index, row in enumerate(group, 1):
            ...
            world_id = f"g10v15_{split}_w{local_index:02d}_{suffix}"
    ...
    manifest = {
        **{key: value for key, value in source_manifest.items() if key not in ("worlds", "assets", "negative_assets")},
        ...
        "approach_sequence": {
            ...
            "capture_frames": 180,
            "minimum_motion_gate_travel_m": 3.58,
    ...
    qa = {
        ...
        "gates": {
            ...
            "namespaced_assets_only": all(row["model_name"].startswith("g10v15_") for row in asset_rows + negative_rows),
