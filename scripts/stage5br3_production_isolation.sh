...
set -u
ROOT=/stage5br3
OUT=${ROOT}/artifacts/stage5br3_20260720_review/production_isolation
mkdir -p "${OUT}"
...
for _ in $(seq 1 80); do gz service -l 2>/dev/null | grep -q '/world/world_a_asphalt_campus/create' && break; sleep .25; done
ros2 run ros_gz_sim create -world world_a_asphalt_campus -file "${OUT}/production_default.urdf" -name sanitation_vehicle -x -8 -y 0 -z .18 >"${OUT}/spawn.log" 2>&1
sleep 5
gz topic -l | sort >"${OUT}/runtime_topics.txt"
python3 scripts/stage5br3_production_isolation.py --root "${ROOT}" --rendered-urdf "${OUT}/production_default.urdf" --runtime-topics "${OUT}/runtime_topics.txt" --output "${OUT}/production_isolation_report.json"
