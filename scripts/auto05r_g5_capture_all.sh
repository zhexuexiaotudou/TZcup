...
REPO=/repo
DATA_ROOT="${AUTO05R_DATA_ROOT:-/data/g5_sealed_native}"
RUNTIME_WS="${AUTO05R_RUNTIME_WS:-/data/runtime_ws_g5}"
...
ros2 run sanitation_learning auto05r_generate_g5_sealed \
  --development-registry "${REPO}/starter_ws/src/sanitation_learning/config/g4_asset_registry.yaml" \
...
capture_world() {
  local world_id="$1"
  local world_index="$2"
  local seed_start=$((5000 + world_index * SCENES_PER_WORLD))
  ...
  for _ in $(seq 1 120); do
    if gz service -l 2>/dev/null | grep -q "/world/${world_id}/create"; then ready=true; break; fi
    sleep .25
  done
  if [[ "${ready}" != true ]]; then echo "world service did not become ready: ${world_id}" >&2; return 2; fi
  ...
  local index seed scene out attempt capture_pass
  for index in $(seq 0 $((SCENES_PER_WORLD - 1))); do
    seed=$((seed_start + index)); scene=$(printf 'scene_%04d' "${seed}")
    out="${DATA_ROOT}/scenes/${scene}"; mkdir -p "${out}/attempts"
    if [[ -f "${out}/capture_report.json" ]] &&
      ...
      echo "resume-skip ${world_id} ${scene}"; continue
    fi
    ...
    sleep 3; capture_pass=false
    for attempt in 1 2 3; do
      ros2 run sanitation_learning auto05r_randomize_g4_scene \
        --manifest "${WORLD_MANIFEST}" --world-id "${world_id}" \
        --scene-seed "${seed}" --scene-index "${index}" \
        --output "${out}/scene_manifest.json" >"${out}/randomize.log"
      sleep 2
      ...
      [[ ! -f "${out}/capture_report.json" ]] || cp "${out}/capture_report.json" \
        "${out}/attempts/capture_report_attempt_${attempt}_failed.json"
      sleep 3
    done
    if [[ "${capture_pass}" != true ]]; then echo "scene failed after 3 attempts: ${scene}" >&2; return 2; fi
  done
  cleanup_world
}
...
echo "G5 sealed capture complete: start_world_index=${START_WORLD_INDEX} worlds=${#WORLD_IDS[@]}"
