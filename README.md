...
| 学习感知 | 修复后的 A1 正式失败，A2 根因修复正式重跑中；冻结/G5 保持锁定 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`。bbox size/ltrb 直接监督修复后，A1 的四模型训练、产品资格和 ONNX 门均通过，但正式 P4 仍在 false candidate/min `14.4`、in-domain macro recall `0.8837`、small-object recall `0.2889`、boundary F1 `0.6913` 四项失败。开发集诊断确认小目标损失主要来自 discovery，且训练抽样只保留 102 个可用小目标中的 22 个；A2 重跑因此保留全部稀缺小目标帧、启用 P3/P4/P5 尺度唯一分配与独立 quality 监督，area checkpoint 改按 IoU/边界联合目标选择。若进入 A3，只允许在同 QA、逐任务正式合格且 training-complete 的条件下复用 A2 classifier/leaf/puddle，A3 discovery 必须重训，所有源报告/checkpoint 哈希写入新报告。P4 与产品 runtime 的候选上限统一为 top-16，classifier 固定 batch-16 并每帧单次 I/O-binding 推理。P6 已补齐真实 discovery/classifier/leaf/puddle 后处理、depth/map 投影、多区域 polygon、tracker v2 和 ROS publisher，但尚未用冻结模型做 Gazebo live 验收。`freeze_auto05r_models.py`、冻结 evaluator 与产品 manifest 生成器继续 fail-closed；P4 失败时不得冻结或访问 G5。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
...
- `scripts/generate_perception_product_status.py`：从显式通过证据生成 P14 状态、证据哈希索引和 blocker register；缺失或 false 证据不会提升任何产品状态；
