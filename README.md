...
| 学习感知 | A1/A2 正式失败，最后登记的 A3 待运行；冻结/G5 保持锁定 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`。A1 正式 P4 在 false candidate/min `14.4`、in-domain macro recall `0.8837`、small-object recall `0.2889`、boundary F1 `0.6913` 四项失败。保留全部稀缺小目标并启用尺度唯一分配后的 A2 仍因 discovery in-domain recall `0.0758`、small-object recall `0` 等门失败；但 classifier/leaf/puddle 均取得 training-complete 合格 checkpoint，area in-domain/cross-world mIoU 为 `0.9222/0.9205`，四 ONNX parity 全过。A3 只允许在同 QA、逐任务正式合格的条件下复用这三个模型，discovery 必须从头做 teacher distillation，全部源报告/checkpoint 哈希写入新报告。P4 与产品 runtime 的候选上限统一为 top-16，classifier 固定 batch-16 并每帧单次 I/O-binding 推理。P6 已补齐真实后处理、depth/map 投影、多区域 polygon、tracker v2 和 ROS publisher，但尚未用冻结模型做 Gazebo live 验收。P4 失败时不得冻结或访问 G5；旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
