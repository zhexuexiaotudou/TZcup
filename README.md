...
| 学习感知 | V5 正式 G4 与 teacher 通过，产品 A3 筛选中 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过，跨 split exact/pHash 重复均为 0；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`，证明数据可学。A1 在 600 帧、8/8 个训练世界上跑满 30 epoch 后最佳 recall/AP50 仅 `0.2069/0.2079`；A2 MobileNetV3-FPN 跑满 30 epoch 后最佳 recall/AP50 均为 `0`，两者均已按硬门淘汰且不再追加 epoch。现在推进最后一个注册架构 A3：冻结 FCOS teacher 的 train-only soft quality target，并将每个 box 唯一分配到 P3/P4/P5，消除三层重复目标冲突。DeepLab 面积对照的 in-domain/cross-world mIoU 为 `0.9273/0.9378`、boundary F1 为 `0.7301/0.7294`，仅用于选择面积架构，不计作 P4。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
