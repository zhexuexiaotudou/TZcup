...
| MODEL-RECOVERY-V2 | MRV2-A/B 正式静态筛选失败，MRV2-C 已解锁 | 历史 X1/X3 仍失败、X2 仍是历史外部阻断。MRV2-A 的 R960 TRAIN-only 重采样/增强在 VAL 得到 macro F1 `0.9450`、metal_can recall `0.9063`，但 small recall `0.4103`、false candidates/min `21.6`，跨域 small recall `0.4737`；MRV2-B 的有界 ground-ROI tile 没有增加有效候选，指标与 A 相同。两条路线均未冻结，最后一条 MRV2-C 教师辅助恢复已按协议解锁；官方 Grounding DINO checkpoint 仅完成新协议来源/结构核验，尚未 benchmark，G5/D6 仍未读。 |
