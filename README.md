...
G2 Area V2 清单把源 `negative_only` 声明传播到 scene/逐 frame，并锁定两 split 的 negative scene/frame/world 计数；negative 声明与对象或 semantic GT 冲突会 fail-closed。三条冻结 Area 路线已有 G2-bound 终态：历史 exact leaf/puddle ONNX 实体在允许根中缺失；eWaSR 在 TRAIN/HOLDOUT 负例上均 `10/10` 激活 native water（像素 `47.95%/41.25%`）；SegFormer-B0 安全执行但 exact water/tree/grass/plant 激活为 0，且无目标类映射、许可不放行。故 `area_screening_complete=true` 只表示有界候选池关闭，Area IoU/迁移学习/PC/产品/J6 仍未授权。
