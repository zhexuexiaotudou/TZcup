...
中心走廊复跑进一步发现正例实物会在车辆完成 180 帧前与侧刷接触；采集器因此把 CLI `--linear-speed-mps` 明确落实为 motion profile 的线速度上限。后续只允许新鲜外置根上的 `g10_route_v19_spin_xneg1p5`：以 `.05 m/s` 直行至 odom `x=-1.50 m` 后零线速原地旋转，360 帧、相邻位移 `.02 m`/旋转 `.03 rad`，并把 route ID/config SHA、固定 domain SHA 纳入 scene/report/trajectory/COCO/QA 身份；不按目标 GT 分支，也不改 world、asset、camera 或碰撞体。该路线尚未形成正式 TRAIN/HOLDOUT 通过证据，旧 route v18 原样保留。
