...
| 学习感知 | V5 正式 G4 与 teacher 通过，修复 discovery 尺寸回归后重跑 A1 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`。A1/A2/A3 的旧运行暴露出共享实现 bug：高分 proposal 中心正确，但预测框塌缩到 4–8 px，因为 loss 缺少 bbox size/ltrb 的直接回归项，仅靠弱 GIoU 约束；因此旧失败保留为诊断，不计作架构上限结论。已补 masked SmoothL1 size loss 与独立梯度测试，下一步按同一正式协议重跑 A1 repaired control。DeepLab 面积对照的 in-domain/cross-world mIoU 为 `0.9273/0.9378`、boundary F1 为 `0.7301/0.7294`，仅用于选择面积架构，不计作 P4。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
