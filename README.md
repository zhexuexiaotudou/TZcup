...
OPRV3 在线优先恢复现已建立独立 evaluator 事件语义、门槛溯源和真实 Gazebo 移动相机开发矩阵：旧的 X1/X3、MRV2-A/B/C 静态失败与 `<18 px` 指标完整保留；产品开发另从相机、速度、减速度、刷盘位置和目标物理尺寸预冻结 `ActionableObservationWindow`，按对象统计 eventual detection/classification/confirmation 与漏清扫机会。当前 24 条基础任务（20 正向 + 4 negative-only，2160 帧、4 个 world）中，MRV2-A 在 99 个具有至少 3 个实际可行动采样帧的目标上 eventual detection/classification 均为 `1.000`、三帧确认 `0.9798`、错误可行动率 `0.00967`；100 个 GT 中另有 1 个因采集节奏不足而独立报告。新增转弯入视野与显式遮挡任务均为 90/90 帧，MRV2-A 对各自 5 个 eligible 目标的 eventual detection/classification/confirmation 均为 `1.000` 且错误动作均为 0；正式湿地反射任务两次仅达 79/90、77/90（RTF 约 `0.042`），故反射覆盖与 area 正式 mIoU/boundary 门仍未通过，`MODEL_BLOCKED_INTERNAL=true`、所有 freeze/sealed/30-seed/spot-clean/soak/J6/field/competition 状态保持 false。协议与复现边界见 [`docs/online-first-recovery-v3.md`](docs/online-first-recovery-v3.md)。
