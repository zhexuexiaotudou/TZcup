...
| 学习感知 | A1/A2/A3 三条正式路线均失败；架构尝试已用尽，冻结/G5 保持锁定 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`。A1 在 false candidate/min `14.4`、macro recall `0.8837`、small-object recall `0.2889`、boundary F1 `0.6913` 失败；A2 discovery in-domain recall 仅 `0.0758`。最后登记的 teacher-distilled A3 虽严格复用了 A2 的三个合格非 discovery checkpoint，且四 ONNX parity/零 custom op 均通过，但 discovery in-domain candidate recall 仅 `0.0136`、macro recall/F1 `0.0111/0.0219`、small-object recall `0`，P4 仍为 false。P4 与产品 runtime 候选上限已统一为 top-16，classifier 固定 batch-16；P6 已补齐真实后处理、depth/map 投影、多区域 polygon、tracker v2、ROS publisher 和 fail-closed 故障矩阵，但没有合格冻结模型，不能做 Gazebo live/soak/发布验收。固定协议禁止新增 A4、降门槛、冻结或访问 G5；旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
