...
| 学习感知 | V5 正式 G4 与 teacher 通过，产品 A1 筛选中 | 新 G4 采用“逐帧零额外目标 + 每个声明类别在十帧序列至少完整可见 2 帧”和显式 `V5_retracted` 位姿覆盖，不修改 Xacro 默认值。最终 12 world / 300 scene / 3000 frame 全部门通过、错误为空、跨 split exact/pHash 重复均为 0；1× FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`，证明数据可学。新增 D1–D5 各 100 帧原生 Gazebo 单因素诊断，独立 QA 全部通过。真正 P3/P4/P5 A1 已恢复约 0.97 discovery recall，但仍有 proposal flood；审计并修复 VAL-only 阈值选择、完整 P4 质量约束、不可行 epoch 过早停止、单任务 boundary selector、任务级 ONNX 聚合、预测框裁剪、训练 600 帧只顺序覆盖 3/8 train worlds 的抽样错误，以及长训仅在正常退出时保存 checkpoint 的中断风险。DeepLab 面积对照的 in-domain/cross-world mIoU 为 `0.9273/0.9378`，boundary F1 为 `0.7301/0.7294`，已选作下一轮全新四模型 A1 的面积架构；该对照复用旧离散 checkpoint，绝不计作 P4。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
