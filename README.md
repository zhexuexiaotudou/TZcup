...
| 学习感知 | 修复后的 A1 正式 P4 运行中，冻结/G5 工具 fail-closed 就绪 | 新 G4 的 12 world / 300 scene / 3000 frame 数据门全部通过；FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`。修复 bbox size/ltrb 直接监督后，同一正式协议下 A1 discovery 已达到 recall `0.9224`、AP50 `0.9208`、precision `1.0`、false candidate/min `0`，classifier macro F1 `0.9733`；leaf 已冻结最优 epoch 22（IoU `0.9790`、boundary F1 `0.7851`、negative-area FP `0`），puddle 已越过约束但仍在预注册训练/早停中，因此此处不提前宣称 P4。新增 `freeze_auto05r_models.py`、冻结 evaluator 与产品 manifest 生成器，强制绑定 P4/G4/预训练权重/checkpoint/ONNX/evaluator 哈希，并在消费一次性 G5 access 前完成无数据 preflight；P4 只能生成 screening claim，匹配 freeze 的 one-shot P5 pass 才能提升 formal claim，live/competition 继续为 false。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
