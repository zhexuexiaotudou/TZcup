...
| MODEL-RECOVERY-V2 | MRV2-A/B 正式静态筛选失败，MRV2-C/官方 benchmark 实现完成 | 历史 X1/X3 仍失败、X2 仍是历史外部阻断。MRV2-A 的 R960 TRAIN-only 重采样/增强在 VAL 得到 macro F1 `0.9450`、metal_can recall `0.9063`，但 small recall `0.4103`、false candidates/min `21.6`，跨域 small recall `0.4737`；MRV2-B 的有界 ground-ROI tile 没有增加有效候选。MRV2-C 已实现 TRAIN-only 高置信教师小目标几何补标、显式 stride-4/P2 六层 FCOS 与闭集三分类训练；官方 Grounding DINO 固定开发集 benchmark 也已实现。当前仍无模型冻结，G5/D6 仍未读。 |
