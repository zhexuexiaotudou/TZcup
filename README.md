...
TF 连续性探针、激光与点云自过滤器统一捕获 ROS 2 launch 的外部关闭信号并使用幂等上下文清理，正常停机不再被二次 `shutdown` 误报为节点故障；建图专用 runner 也显式关闭与产品速度门重复发布的上游 `command_timeout`。每个长驻进程组都必须留下 wrapper 退出码、最终信号阶段和残留扫描的 shutdown 记录；Gazebo 先走 `/server_control stop` 再进入通用信号收口，缺记录、残留或被迫 `SIGKILL` 都会让 Mapping Gate 失败关闭。
