...
G10 approach 模式显式 opt-in，route-v13 使用独立 `g10v13_*` world/asset 命名空间和 6/3/3 个 TRAIN/HOLDOUT/DEV-VAL 世界。每个正 mission 只包含一个约 6.7m 前方候选目标，can/paper/bottle 在各 split 的 mission 间均衡轮换；类别轮换只绑定稳定的源世界后缀与 scene index，禁止 route namespace 改变类别。冻结采集为 150 帧、0.02m 相邻平移门，车辆直行经过候选后在 world x=-0.50m 处锁存左弧线，避开湿地排水障碍。production capture 控制只读 odom，不读取 GT topic；离线 QA 对每个正目标硬性要求按时间顺序覆盖 `<18`、`18-32`、`32-64`，并达到独立冻结的 `MIN_RELIABLE_CLASSIFICATION_SHORT_SIDE_PX=18`。默认 G4/G8 随机化行为不变，DEV-VAL 只在综合 HOLDOUT 通过并冻结完整链路后原子读取。
