...
G10 approach 模式显式 opt-in，route-v15 使用独立 `g10v15_*` world/asset 命名空间和 6/3/3 个 TRAIN/HOLDOUT/DEV-VAL 世界。每个正 mission 只包含一个约 7.1m 前方候选目标，can/paper/bottle 在各 split 的 mission 间均衡轮换；类别轮换只绑定稳定的源世界后缀与 scene index，禁止 route namespace 改变类别。冻结采集为 180 帧、0.02m 相邻平移门，车辆直行经过候选后在 world x=-0.50m 处按已知静态 layout 离场：mixed-curb 与 light-paver world 使用9帧浅左弧后直行，避免车体在持续旋转中扫入路缘或已驶过的目标；其余 world 保持左弧绕开横向障碍。控制只读 odom 与离线 world layout，不读取目标 GT topic；离线 QA 对每个正目标硬性要求按时间顺序覆盖 `<18`、`18-32`、`32-64`，并达到独立冻结的 `MIN_RELIABLE_CLASSIFICATION_SHORT_SIDE_PX=18`。默认 G4/G8 随机化行为不变，DEV-VAL 只在综合 HOLDOUT 通过并冻结完整链路后原子读取。
...
该唯一恢复已真实耗尽且仍失败：macro-F1 从 `0.4941` 提升到 `0.6318`，但 bottle P/R=`0.7509/0.9420`、can P/R=`0.9171/0.7034`、paper P/R=`0.5891/1.0000`、background specificity=`0.0833`，未达到任何完整 classifier 硬门。因此 V10 在 `TASK_REFORMULATION_BLOCKED=true`、`MODEL_BLOCKED_INTERNAL=true` 处 fail-closed 停止；ActionVerifier、integrated HOLDOUT、DEV-VAL、VAL_NEW、G5_V2、release 和部署均未执行。

CRCRV11 进一步确认 proposal crop 几何不是主因：GT/proposal tight macro-F1 仅差 `0.0064`，但 V10 TRAIN 只有 9 个 unique background crop、replacement sampler 期望重复约 `186.64` 次，runtime-faithful positive view 只占 `40%`。C11 将背景扩展到 6,576 个 unique tight crop 并恢复 R1 background specificity 到 `1.0`，但 R1/R2/R3 的目标分类门仍全部失败，最佳正式 candidate macro-F1 仅 `0.6311`。因此 [CRCRV11](docs/close-range-classifier-contract-recovery-v11.md) 按停止条件 B 保持 `MODEL_BLOCKED_INTERNAL=true`，sealed 数据和所有下游产品门继续锁定。
