...
仓库现有一键式未知栅格 frontier 连续建图→保存地图/位姿图→硬停止→全新 Gazebo/定位/Nav2 进程→加载重定位→多航点导航入口。40 m × 20 m 烟测已跑通整条接线并保持两阶段 TF 断裂为 0，但 `formal_scope=false`。Ackermann frontier 已加入独立长前视控制、物理可达转弯、60 s 目标看门狗、180 s 失败记忆、边界耗尽恢复、Nav2 原生碰撞检查倒车，以及仅在正式范围启用的边界/激光量程推导蛇形前沿偏置；该偏置只排序在线 occupancy-grid frontier，不把边界点直接发给 Nav2。横向扫带要求在线地图观测正式 x 边界；垂直换带先用碰撞检查 BackUp 退离边界，再从当前融合位姿、在线 costmap 和扫带方向生成短距已知自由区 staging 候选。连接器由 Smac Hybrid 只规划一次，整条路径再按前进/倒车 cusp 分段，分别交给 `DubinsPath`/`ReversePath`，避免重规划或允许倒车的通用控制器在静止车位改变起始档位。当前最佳正式口径限时证据为 1200.39 s 建图 4958.44 m²（4.13 m²/s，114 成功/4 失败），地图横向真实跨过 x=±100 m、TF 断裂为 0，较旧 3741.1 m² 提升约 32.5%。但 plan-once 换带的 6/4/2 m 在线候选均在 forward 分段触发 Nav2 进度失败，纵向地图边界仍停在约 32 m；因此 20,000 m² 全程及其保存、硬重启、加载重定位、多航点正式链未执行，Mapping Gate 继续失效关闭。完整感知、Tracking、Spot Cleaning 与 re-observation 流水线同样因近距分类停止条件 B 未执行。
