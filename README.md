...
长时无头多种子回归使用 `scripts/run_frozen_coverage_trial.ps1`：每轮固定独立 ROS domain、Gazebo partition 和小场任务配置，通过 `Start-Process -Wait` 监督 WSL，并分开保存 stdout/stderr 启动日志与证据目录；普通 ROS stderr 警告和测试编排器的会话寿命都不会冒充任务终止。
