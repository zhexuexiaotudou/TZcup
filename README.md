...
优化方案将清扫直线固定为 `CleanPath`（`0.65 m/s`、固定前视），补扫使用 `RepairPath`，转向和横移使用 Nav2 `Spin / DriveOnHeading / BackUp`；补扫中心线只覆盖漏扫连通域，按圆形刷盘已覆盖的端帽向内收紧，并复用主条带的离线法向标定；长度按实际下发路径而非名义残余线验收，`fast/turbo` 只改变 Gazebo 推进倍率，不再把所有物理动作一并加速。
