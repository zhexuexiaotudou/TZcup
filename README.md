...
OPRV3 在线优先恢复现已建立独立 evaluator 事件语义、门槛溯源和真实 Gazebo 移动相机开发矩阵：旧的 X1/X3、MRV2-A/B/C 静态失败与 `<18 px` 指标完整保留；产品开发另从相机、速度、减速度、刷盘位置和目标物理尺寸预冻结 `ActionableObservationWindow`，按对象统计 eventual detection/classification/confirmation 与漏清扫机会。24 条基础任务加转弯入视野、显式遮挡、湿地反射三条独立任务共保留 115 个 GT，其中 114 个进入预冻结可操作窗口；MRV2-A eventual detection/classification 均为 `1.000`、三帧确认 `0.9737`、错误可行动率 `9/1113=0.00809`、漏清扫机会为 0，全部要求的 moving coverage 已有 GT 实证，故 OPRV3-01/02 通过并进入 OPRV3-07。首次 OPRV3-07 fail-closed 审计确认对象级在线发现五项门全部通过，但 Area 的 puddle IoU `0.7222`、boundary F1 `0.6388`、负场景 FP/frame `0.1304` 未达门槛，Map/Track、正式流水线性能以及两项错误行为证据仍为空并按失败处理；下一步回到 OPRV3-06 Area 恢复，`MODEL_BLOCKED_INTERNAL=true`、`OPRV3_X86_DEV_PASS=false`，所有 freeze/sealed/30-seed/spot-clean/soak/J6/field/competition 状态保持 false。协议与复现边界见 [`docs/online-first-recovery-v3.md`](docs/online-first-recovery-v3.md)。
