...
| 学习感知 | V5 正式 G4 与 teacher 通过，产品 A1 筛选中 | 新 G4 采用“逐帧零额外目标 + 每个声明类别在十帧序列至少完整可见 2 帧”和显式 `V5_retracted` 位姿覆盖，不修改 Xacro 默认值。最终 12 world / 300 scene / 3000 frame 全部门通过、错误为空、跨 split exact/pHash 重复均为 0；1× FCOS-R50 teacher 在正式 val 达到 recall `0.9554`、AP50 `0.9505`、precision `1.0`、false candidate/min `0`，证明数据可学。新增 D1–D5 各 100 帧原生 Gazebo 单因素诊断，独立 QA 全部通过。修正 checkpoint 选择后，旧单网格融合 A1 仍以 discovery/leaf 退化失败；现按预注册 A1 路线运行真正独立 P3/P4/P5、EMA warmup 和稀疏边界损失版本，不复用旧 checkpoint。旧 D6/G5 仍未读，`AUTO_05R/P4/P5/formal/live/J6/field` 尚未通过，见 [`docs/auto05r-p2-data-integrity-recovery.md`](docs/auto05r-p2-data-integrity-recovery.md) |
