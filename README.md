...
| MODEL-RECOVERY-V2 | MRV2-00 审计完成，MRV2-A 正式训练已就绪 | 历史 X1/X3 仍失败、X2 仍是历史外部阻断；新审计确认 X3 有效 batch 仅 `34/600` 帧含 `<18 px` 目标，top-K 不是瓶颈，metal_can 主要为低分而非错类，简单 area 后处理不能修复 D4。MRV2-A 已固定 `30% small + 20% negative + 15% metal + 35% general` 配额、TRAIN-only copy-paste/crop-scale/metal 光照增强与 R960/R640 有界比较；官方 Grounding DINO checkpoint 已完成新协议来源/结构核验但尚未 benchmark，G5/D6 仍未读。 |
