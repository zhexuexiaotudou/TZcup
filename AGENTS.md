...
- 发布分支优先使用 `codex/<task-slug>`，PR目标为 `main`。只显式暂存本任务文件。
- 每次改动都必须复核中文 `README.md`，但只有项目定位、使用方法、目录结构或当前能力边界变化时才更新。阶段过程写入 `docs/progress.md` 或对应专题文档，提交过程由 Git/PR 记录；禁止把 README 写成逐步变更日志。
- `shumo` 仅在用户明确提出数学建模问题或数学建模竞赛任务时使用。本项目默认是 ROS 2 / Gazebo 机器人仿真工程，不自动触发 `shumo`。

## 开始任务前必读

- `README.md`：中文项目入口、当前状态、快速开始和能力边界。
...
- PR必须通过 `.github/workflows/development-workflow.yml` 中的“开发工作流”CI；受影响的 Stage 运行证据写入 PR说明或可追溯的 artifact。
