...
section('01 项目背景与交付边界','面向智慧环卫场景的国产系统无人清扫车关键技术攻关；赛题DG-202604。本报告针对一天内的最小交付，采用独立软件仿真演示与本地S100P算法执行证据两条路线。当前材料具备可审阅正文，完整仿真Demo仍缺，不能宣称全部达标。','需求来自包内 contest-brief.txt 第2–4页与第6页。六项功能与五项数字是本次收口工作范围，不能替代题面全部要求。报告还披露识别准确率≥95%、尘箱≥40L、两种交互、任务分解等未测条目。','证据截至 '+STAMP+'。本包由旧main建立的独立工作树生成，未构建或运行机器人；本工作树源码不是最新仿真运行源码。历史板端版本6731784；最新状态页候选1a211400，仅本地候选就绪。')
...
section('19 失败、限制与其他题面要求','保留首图45分钟未封图、progress_timeout恢复、历史转向定位偏差与当前生命周期/录包问题。execution-status-snapshot.md是历史状态的快照，不表示后台后续工作停止。当前完整任务尚无成功分母，不能合并不同版本的组件PASS。','额外题面要求：识别准确率≥95%、尘箱承载≥40L、两种交互及指令准确率≥95%、任务分解正文≥95%而评分≥90%、新手3天、日均故障率<1%、三步更换、积水/落叶、边界防护。此次未新增测试，均披露NOT_MEASURED，不归类为自动取消的加分项。','当前提交风险：缺Demo视频、报名表、申报人命名信息及当前仿真结果；技术报告可供审阅，但不得把整包称为已符合全部比赛最低要求。题面有关键技术指标不满足的一票否决说明。')
section('20 提交格式、证据接入与封包','项目保存题面要求：技术方案PDF 20–50页；软件仿真或实物二选一；演示视频5–10分钟并含操作说明；审核通过的参赛报名表；压缩包名为“申报人所在单位-申报人姓名-作品名称-联系电话”。未查到单个PDF或视频的强制文件名，当前采用项目自定描述名。','可立即提交的内容材料：本20页技术方案、唯一矩阵、板端回放证明与失败限制。这表示材料可取用，不表示整包可提交通过。等待：完整任务关键JSON/日志、地图/轨迹与测量数据、截图、5–10分钟视频、报名表及最终身份信息。','不再做的加分或工程增强：选交源代码/数据集打包、用户调研/产业化方案、专利论文补充、500图与正式HBM补制、长稳、三次任务、GitHub/PR/部署/复杂取证。已有真实证据仍保留。取得结果后预计5–10分钟刷新矩阵与PDF、5–10分钟复核与压缩；不含补测、视频制作与报名表取得时间。')
