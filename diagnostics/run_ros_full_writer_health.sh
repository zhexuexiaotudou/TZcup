...
E=/mnt/f/Project/TZcup/.workspace/evidence/short09-ros-full-writer-01
[ ! -e "$E" ]; mkdir "$E"
...
set +e
timeout --signal=TERM --kill-after=10s 60s python3 -B "$W/diagnostics/ros_full_writer_health_test.py" --observer "$W/diagnostics/verify_live_frontier_short_09.py" --output "$E/run" > "$E/test.log" 2>&1
rc=$?
set -e
printf '%s\n' "$rc" > "$E/test.rc"
exit "$rc"
